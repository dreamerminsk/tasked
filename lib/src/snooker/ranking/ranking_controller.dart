      ...
      if (table != null) {
        ...
        if (tbody != null) {
          ...
          for (final row in rows) {
            final position = row.querySelector('.position')?.text.trim() ?? '';
            final playerElement = row.querySelector('.player a');
            final player = playerElement?.text.trim() ?? '';
            final playerId = playerElement?.attributes['href']?.split('=')?.last ?? '';
            final nationality = row.querySelector('.nationality')?.text.trim() ?? '';
            final sum = row.querySelector('.sum')?.text.trim() ?? '';
            final sumChange = row.querySelectorAll('.change').last?.text.trim() ?? '';
            print('Position: $position, Player: $player, ID: $playerId, Nationality: $nationality, Sum: $sum, Sum Change: $sumChange');
          }
        } else {
